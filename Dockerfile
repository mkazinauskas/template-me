...
# LibreOffice + fonts come from `base` — needed for LOCAL_MODE's docx->pdf
# conversion (see src/lib/docx-to-pdf.ts), which shells out to `soffice`
# instead of booting a Vercel Sandbox. See `base` for the font rationale.
FROM base AS runner
WORKDIR /app
