...
describe("ClientTemplatePage routes", () => {
  it("delegates the template's own route to <TemplateDetail> with the warnings param", async () => {
    const { default: Page } = await import("@/app/client/dashboard/templates/[id]/page");
    render(
      await Page({
  ...
  it("renders the bulk mode at /[id]/bulk", async () => {
    const { default: Page } = await import("@/app/client/dashboard/templates/[id]/bulk/page");
  ...
  it("renders the send mode at /[id]/send", async () => {
    const { default: Page } = await import("@/app/client/dashboard/templates/[id]/send/page");
    render(await Page({ params: Promise.resolve({ id: "t1" }) }));
  ...
  it("builds each route's metadata from the shared helper, tagged with its mode", async () => {
    const own = await import("@/app/client/dashboard/templates/[id]/page");
    const bulk = await import("@/app/client/dashboard/templates/[id]/bulk/page");
    const send = await import("@/app/client/dashboard/templates/[id]/send/page");
